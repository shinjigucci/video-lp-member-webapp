...
目的は、広告やSNSから来た見込み客に、商品価値を約10分で伝え、LINE登録、セミナー申込、個別相談などの次の行動につなげる動画LP制作物を作ることです。
...
- 視聴者本人を責めず、問題を「順番」「導線」「文脈」「受け皿」の問題として説明してください。
- 出力は日本語のみ。
...
この10段階の目的:
- いきなり商品を売るのではなく、視聴者の認識を変える。
- 「自分に関係ある」「今までのやり方がズレていたかもしれない」「本当の問題はそこだったのか」「この解決策なら知りたい」「自分にもできそう」「まず無料特典を受け取ろう」という順番で進める。
- 広告・SNSと申込ページの間に置く「理解の橋」として機能させる。
...
各フックは3〜5文で構成してください。
冒頭10秒以内に「見る理由」または「成果約束」を入れてください。
できるだけ自己紹介から始めず、視聴者の悩み・常識・欲しい変化から入ってください。
...
必ず次のJSONだけを返してください。
{
  ...
  "top10": [
    {"id": "TOP10-1", "sourceId": "1-1", "type": "成果約束型", "hook": "", "reason": ""}
  ],
  "byType": [
    {
      "typeNo": 1,
...
絶対条件:
- fullScriptの10項目合計を必ず3,700〜4,300文字にしてください。
- avatarScriptも3,700〜4,300文字にしてください。
- すべて、AIアバターがそのまま話せる自然な話し言葉にしてください。
...
fullScriptの10段階と文字数目安:
- promiseHook: 120〜180文字。冒頭10秒で見る理由と成果約束を作る。
- beliefBreak: 220〜320文字。見込み客の常識を揺らす。
- realProblem: 400〜550文字。表面的な悩みの奥にある本当の問題を言語化する。
- namedSolution: 350〜500文字。独自名称つき新解決策として記憶させる。
- analogy: 320〜460文字。日常比喩で一瞬で腹落ちさせる。
- caseStory: 750〜950文字。問題、間違い、視点転換、変化、原理まで示す。
- futureSimulation: 350〜500文字。自分にもできそうと思わせる。
- proofTrust: 450〜650文字。根拠、信頼、再現性、反論処理を入れる。
- freeOfferAction: 280〜420文字。無料特典を最初の一歩として見せる。
- philosophyClose: 380〜560文字。思想化してから具体的CTAで締める。
...
- 40〜44枚: できる未来の疑似体験
- 45〜50枚: 根拠・信頼・再現性
- 51〜55枚: 無料特典と次の行動、思想化と限定クロージング

imagePromptの共通条件:
- 横長16:9のビジネスセミナー用スライド
- 白、濃紺、黒を基調に、青またはゴールドをアクセント
- スマホでも読める大きな文字
- 1スライド1メッセージ
...
このステップではスライド構成を作らないでください。台本の厚みと10段階構造の再現性を最優先してください。

絶対条件:
- fullScriptの10項目合計を必ず3,700〜4,300文字にしてください。
- avatarScriptも必ず3,700〜4,300文字にしてください。
- 短い要約、箇条書き、説明メモで終わらせないでください。
- すべて、実際にAIアバターがそのまま話せる自然な話し言葉で書いてください。
- 自己紹介から始めず、選択フックから自然に入ってください。
- 商品をその場で売る長いセールストークではなく、無料特典、LINE登録、セミナー申込、個別相談などの次の行動へ進める台本にしてください。
- 視聴者本人を責めず、問題を「順番」「導線」「文脈」「受け皿」の問題として説明してください。
- 解決策には必ず独自名称をつけてください。
- 日常比喩を必ず1つ入れてください。
- 事例変化ストーリーを必ず入れてください。実績が不明な場合は「たとえば」という仮想事例として、成果を捏造しないでください。
- 「これはセンスではなく型です」という再現性メッセージを入れてください。
- 無料特典は単なる資料ではなく「公式」「設計図」「最初の一歩」として見せてください。
- 最後は思想化してから、具体的なCTAで締めてください。
- 返答はJSONだけです。
...
function isProValid(result) {
  ...
  return metrics.fullScriptChars >= 3700
    && metrics.avatarScriptChars >= 3700
...
function isScriptValid(result) {
  const metrics = proMetrics(result);
  return metrics.fullScriptChars >= 3700 && metrics.avatarScriptChars >= 3700;
}
...
現在のfullScript文字数: ${metrics.fullScriptChars}文字。最低3,700文字、目標3,700〜4,300文字。
現在のavatarScript文字数: ${metrics.avatarScriptChars}文字。最低3,700文字、目標3,700〜4,300文字。

修正ルール:
- 箇条書きではなく、実際に話せる本文として増やしてください。
- 10段階それぞれで、見込み客の心理、具体例、誤解の解除、次の行動理由を入れてください。
- 成果約束、常識破壊、問題の正体、独自名称、日常比喩、事例変化、疑似体験、根拠、無料特典、思想クロージングの順番を崩さないでください。
- 同じ文の繰り返しで増やさないでください。
- 返答はJSONだけです。
